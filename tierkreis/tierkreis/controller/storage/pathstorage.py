...
class PathStorageBase(PathStorageBackend):
    ...
    def read_worker_call_args(self, node_location: Loc) -> WorkerCallArgs:
        ...
        return WorkerCallArgs(**json.loads(self._read(node_definition_path)))
    ...
    def write_output(
        ...
        self._write(output_path, bytes(value))
        return output_path
    ...
    def write_metadata(self, node_location: Loc) -> None:
        ...
        self._write(self._metadata_path(node_location), j.encode())

    def read_metadata(self, node_location: Loc) -> dict[str, Any]:
        return json.loads(self._read(self._metadata_path(node_location)))

    def read_started_time(self, node_location: Loc) -> str | None:
        node_def = Path(self._nodedef_path(node_location))
        if not self._exists(node_def):
            return None
        since_epoch = self._stat(node_def).st_mtime
    ...
    def read_finished_time(self, node_location: Loc) -> str | None:
        ...
        if not self._exists(done):
            return None
        since_epoch = self._stat(done).st_mtime
        return datetime.fromtimestamp(since_epoch).isoformat()
